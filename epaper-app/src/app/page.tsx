...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 flex flex-col font-sans select-none overflow-x-hidden">
      ...
      {/* 🔴 Z-INDEX FIX: Header ko sabse upar rakha hai taaki uske clicks block na hon */}
      <div className="relative z-50">
        <Header
          ...
          // MASTER PROP FIX: Header kisi bhi naam se data maange, use mil jayega!
      ...
      <div className="flex-1 flex flex-col md:flex-row relative overflow-hidden my-2 px-2 gap-2 z-10">
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8 text-center bg-slate-900/40 rounded-3xl border border-slate-800/80 my-8">
            <div className="w-20 h-20 bg-slate-800 rounded-full flex items-center justify-center mb-4 text-emerald-500 font-bold text-2xl">🗞️</div>
            <h2 className="text-xl font-bold text-slate-200 mb-2">No Newspaper Published for {selectedDate}</h2>
            <p className="text-sm text-slate-500 max-w-md">Is date ke liye abhi koi newspaper upload nahi hua hai. Kripya kisi doosri date ko select karein.</p>
          </div>
        )}
      </div>
